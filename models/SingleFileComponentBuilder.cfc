	...
	function build( required componentPath, required componentName, module = "" ){
		...
		if ( fileExists( local.cfmPath ) ) {
			...
			return loadComponent(
				...
				listLast( local.files.tempComponentName, "." ),
				arguments.module
			);
		}
	}
	...
	function parseContents( required cfmPath ){
		...
		if ( !local.startedWire || !local.endedWire ) {
			throw(
				...
				message = "The CBWIRE component '#arguments.cfmPath#' is missing '//@startWire' and '//@endWire' markers. Please place these in your CFSCRIPT block and place each one on a single line."
			);
		}
