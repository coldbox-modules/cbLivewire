component singleton {
	...
	function wire(
		...
		return arguments.lazy ? local.instance._generateXIntersectLazyLoadSnapshot( params = arguments.params ) : local.instance._render();
	}
	...
	function handleFilePreview( incomingRequest, event ){
		...
		local.contents = fileReadBinary(
			getCanonicalPath( variables.moduleSettings.moduleRootPath & "models/tmp/#local.metaJSON.serverFile#" )
		);
		event
			.sendFile(
				file = local.contents,
	...
	function createInstance( name ){
		...
		if ( find( "@", local.fullComponentPath ) ) {
			...
			if ( params.len() != 2 ) {
				throw(
					type = "ModuleNotFound",
					message = "CBWIRE cannot locate the module or component using '" & local.fullComponentPath & "'."
				);
			}
		...
		try {
			...
			if ( variables._singleFileComponents.contains( arguments.name ) ) {
				throw(
					...
					message = "Component '#arguments.name#' is a single file component."
				);
			}
			// Attempt to create an instance of the component
			local.componentInstance = variables.wirebox
				.getInstance( local.fullComponentPath )
				._withPath( arguments.name );
			return local.componentInstance;
	...
	function getModuleComponentPath( path, module ){
		...
		var result = arguments.path contains "." ? moduleRegistry[ module ].invocationPath & "." & module & "." & arguments.path : moduleRegistry[
			module
	...
	function generateSignedUploadURL(){
		...
		return local.baseURL & "/cbwire/upload?expires=" & local.expires & "&signature=" & urlEncodedFormat(
			local.signature
		);
	}
	...
	function getCurrentRequestModule(){
		...
		return structKeyExists( local.rc, "fingerprint" ) ? local.rc.fingerprint.module : variables.requestService
			.getContext()
			.getCurrentModule();
	}
	...
	function view(
		...
		return requestService
			.getController()
			.getRenderer()
			.view( argumentCollection = arguments );
	}
