	...
	function handle( content ){
		...
		local.instanceMatches.each( function( _match ){
			...
			if ( len( trim( local.additionalArgs ) ) && !reFindNoCase( argsPattern, local.additionalArgs ) ) {
				throw(
					type = "CBWIREParseException",
					message = "Parsing error: unable to process cbwire tag due to malformed attributes."
				);
			}
			...
			content = content.replaceNoCase(
				_match,
				"##wire( name=""#local.componentName#"", params=#paramsString#, lazy=false )##"
			);
		} );
