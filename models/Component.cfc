    ...
    */
	...
	function wire(
        ...
        */
		...
		local.instance = variables._CBWIREController
    ...
    */
	...
	function _hydrate( componentPayload ){
        ...
        */
		variables.data.each( function( key, value ){
			if (
				isArray( arguments.value ) && arguments.value.len() && isSimpleValue( arguments.value.first() ) && arguments.value[
					1
				] contains "fileupload:"
			) {
	...
	function _applyUpdates( updates ){
		...
		arguments.updates.each( function( key, value ){
			...
			if ( reFindNoCase( "\.[0-9]+", arguments.key ) ) {
				...
				variables.data[ local.propertyName ][ local.arrayIndex + 1 ] = isNumeric( arguments.value ) ? val(
					arguments.value
				) : arguments.value;
		...
		if ( structKeyExists( this, "onUpdate" ) ) {
			invoke(
				...
				"onUpdate",
				{
					newValues : duplicate( variables.data ),
					oldValues : local.oldValues
				}
			);
		}
	}
	...
	function _getValidationManager(){
		try {
			...
			throw(
				...
				message = "ValidationManager not found. Make sure the 'cbvalidation' module is installed."
			);
		}
	}
	...
	function _insertInitialLivewireAttributes( html, snapshotEncoded, id ){
		...
		if ( local.openingTagStart > 0 && local.openingTagEnd > 0 ) {
			local.openingTag = mid(
				arguments.html,
				local.openingTagStart,
				local.openingTagEnd - local.openingTagStart + 1
			);
			local.newOpeningTag = replace(
				local.openingTag,
				"<" & local.outerElement,
				"<" & local.outerElement & " " & local.wireAttributes,
				"one"
			);
	...
	function _insertSubsequentLivewireAttributes( html ){
		...
		local.outerElement = _getOuterElement( arguments.html );
		// Insert attributes into the opening tag
		return arguments.html.reReplaceNoCase(
			"<" & local.outerElement & "\s*",
			"<" & local.outerElement & " " & local.wireAttributes & " ",
			"one"
		);
	}
	...
	function _validateSingleOuterElement( trimmedHtml ){
		...
		local.tags.each( function( tag, index ){
			...
			if ( depth == 0 && index != tags.len() || index == tags.len() && depth != 0 ) {
				throw(
					"CBWIRETemplateException",
					"Template has more than one outer element, or is missing an end tag </element>."
				);
			}
		} );
	}
	...
	function _generateXIntersectLazyLoadSnapshot( params = {} ){
		...
		if ( isNull( local.html ) || !local.html.len() ) {
			throw(
				type = "CBWIREException",
				message = "The placeholder method must be defined for lazy loaded components and it must have the same outer element as your CBWIRE template."
			);
		}
		...
		// Insert attributes into the opening tag
		return local.html.reReplaceNoCase(
			"<" & local.outerElement & "\s*",
			"<" & local.outerElement & " " & local.wireAttributes & " ",
			"one"
		);
	...
	function _getHTTPResponse( componentPayload, httpRequestState ){
        ...
        */
		...
		local.snapshot.data.each( function( key, value ){
			...
			if (
				isArray( arguments.value ) && arguments.value.len() && isInstanceOf(
					arguments.value[ 1 ],
					"FileUpload"
				)
			) {
	...
	function _prepareIsolation(){
		...
		variables._isolate = variables.keyExists( "isolate" ) && isBoolean( variables.isolate ) && variables.isolate ? true : false;
	}
	...
	function _prepareLazyLoading(){
		...
		variables._lazyLoad = variables.keyExists( "lazyLoad" ) && isBoolean( variables.lazyLoad ) && variables.lazyLoad ? true : false;
        ...
        */
		...
		variables.listeners.each( function( key, value ){
			if ( !variables.keyExists( arguments.value ) ) {
				throw(
					type = "CBWIREException",
					message = "The listener '#arguments.key#' references a method '#arguments.value#' but this method does not exist. Please implement '#arguments.value#()' on your component."
				);
			}
		} );
	}
	...
	function shouldTrimStringValues(){
		return ( _globalSettings.keyExists( "trimStringValues" ) && _globalSettings.trimStringValues == true ) ||
